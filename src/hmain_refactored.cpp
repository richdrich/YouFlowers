...
bool ProcessKeys() {
    ...
    case 'd':
		...
		{
			int camNum = (c=='c') ? 0 : 1;
			bool res = cap[camNum].open(camNum);
			if(res) {
			...
			cap[camNum].set(CV_CAP_PROP_CONVERT_RGB, 1.);
			IsCam = true;
		}
        break;
    case 'f':
    case 'g':
		{
			...
			const char *fname = IoFiles[camNum];

			bool res = cap[camNum].open(fname);
			if(res) {
    ...
    case 't':
		{
			...
			if(IoFiles[camNum] == NULL) {
				Error("No file defined\n");
				break;
			}

			if(!Vwrite[camNum].open(IoFiles[c-'s'], CV_FOURCC('I', 'Y', 'U', 'V'),
...
void ProcessStateFind(Mat &grey) {
...
void DebugDisplayFrame(int state, Mat &rsImage) {
	// debug
	putText(rsImage, state==FIND ? "Find" : (state==ACQUIRE ? "Acquire" : "Track"),
...
void ProcessAcquire(int state, Mat &grey, Mat &prevGrey, bool &averageDone) {
...
//    Mat image;
    ...
    bool chanDone[2] = {false,false};

    while(true) {
    	if(!Paused) {
			for(int ch=0; ch<2; ch++)
			{
				if(chanDone[ch] || !cap[ch].isOpened())
					continue;
				...
				if(state==FIND) {
					ProcessStateFind(grey);

					if(trackPoints[1].size() > 0) {
						printf("Found %d points, state=>ACQUIRE, paused\n", trackPoints[1].size());
						state = ACQUIRE;
						// Paused = true;
					}
				} else if(state==ACQUIRE || state==TRACK) {
					bool averageDone;
					ProcessAcquire(state, prevGrey, grey, averageDone);
				...
				DebugDisplayFrame(state, rsImage);
